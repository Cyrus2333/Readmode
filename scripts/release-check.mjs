...
try { license = await read('src/pro/license.js'); }
  ...
  console.error('[FAIL] 生产许可证模块 — 私有 Store 构建缺少 src/pro/license.js');
  process.exit(1);
}
...
for (const [name, pattern] of urlFields) {
  if (pattern.test(config)) pass(`${name} 已配置`);
...
for (const file of siteFiles) {
  ...
  if (/预发布|待配置|尚未配置|不应据此向用户收费|当前没有可购买|发布前配置|正式发布前|项目负责人|issue tracker/i.test(content)) {
...
if (/尚未接入真实支付|正式商业化前请不要向用户收费/i.test(options)) fail('设置页商业文案', '仍包含预发布支付提示');
else pass('设置页商业文案');
...
console.log('release-check: 商业化发布门槛已通过');
